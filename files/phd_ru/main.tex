...
\cvrow{Награды и\\достижения}{%
  \entry
    {\textbf{Стипендия правительства КНР (CSC) для обучения}}
    {2024--2027}
...
\cvrow{Опыт работы}{%
  ...
  \workprojectdetail{Спроектировал механизм приватного чтения из мирового состояния 
  (ledger world state) в Hyperledger Fabric на основе PIR-протокола с использованием схемы ГШ 
  BGV из библиотеки Lattigo.}
  \workprojectdetail{Разработал модель угроз с honest-but-curious (HBC) узлом
  и показал приватность запросов на основе IND-CPA-безопасности
  BGV, включая повторные запросы и наблюдаемую утечку информации.}
  ...
  \workprojectdetail{Спроектировал архитектуру TWAPPA-FRL и реализовал
  механизм приватной взвешенной агрегации обновлений DQN на базе схемы
  ГШ CKKS с использованием библиотеки TenSEAL.}
  ...
  \workprojectdetail{Разработал модель угроз c honest-but-curious (HBC) агрегатором, показал
  приватность обновлений клиентов на основе IND-CPA-безопасности CKKS, а также наблюдаемую 
  утечку информации.}

  \workproject{\textbf{Верифицируемое многоключевое шифрование с возможностью поиска}}
  \workprojectdetail{Проанализировал модель угроз с HBC-владельцем и пользователем данных,
  и вредоносным облачным сервером; показал приватность запросов в модели
  real/ideal на основе безопасности OT и псевдослучайности CI-rOPRF/PRG
  при явно заданной утечке шаблонов поиска и доступа.}
  % \workprojectdetail{Проанализировал обоснование верифицируемости результатов с помощью
  % проверочного тега ключа и AEAD, позволяющих обнаруживать подмену ключей
  % и файлов вредоносным облачным сервером.}
}
...
\cvrow{}{%
  ...
  \workproject{Построил конвейер обработки ВПО на базе Airflow:
  сбор образцов из MalwareBazaar, хранение артефактов в S3 и сканирование YARA.}
...
\cvrow{}{%
  ...
  \workproject{Реализовал REST API на Python и конвейер генерации курсов на базе LangChain
  с использованием асинхронной оркестрации LLM.}
...
\cvrow{Навыки}{%
  \begin{tabularx}{\linewidth}
    ...
    \skillcell{Стандарты и регуляторика:}{%
      ...
      ФЗ № 63, 149, 152, 187; FIPS 203--205}
    &
