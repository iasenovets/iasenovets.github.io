...
\cvrow{Научные\\интересы}{%
  \textit{Гомоморфное шифрование, приватное извлечение информации,
  приватный поиск шаблона, шифрование с возможностью поиска,
  технологии повышения конфиденциальности, конфиденциальность блокчейна.}
}
...
\cvrow{Образование}{%
  ...
  \edudetail{Средняя оценка: 95/100}
  ...
  \edudetail{Средний балл: 4,45/5,00}
...
\cvrow{Опыт\\исследований}{%
  ...
  Спроектировал и реализовал на Go с использованием Lattigo приватное
  извлечение информации на основе BGV для конфиденциальных запросов
  к мировому состоянию в Hyperledger Fabric; приватность запросов
  в модели HBC-узла основана на IND-CPA-безопасности BGV.}

  \workproject{\textbf{\href{https://github.com/hetiantian10/TWAPPA-FRL}{TWAPPA-FRL}:}
  Спроектировал архитектуру и реализовал с использованием TenSEAL приватную
  агрегацию обновлений модели DQN на основе CKKS; приватность обновлений
  клиентов в модели HBC-агрегатора основана на IND-CPA, при этом явно
  проанализированы утечки информации.}

  \workproject{\textbf{EVMK-SSE:} Проанализировал модель угроз с HBC-владельцем
  данных и пользователем, а также вредоносным облачным сервером; проверил
  аргументацию приватности запросов в реальной/идеальной модели, основанную
  на безопасности OT и псевдослучайности CI-rOPRF/PRG при явно определённых
  утечках шаблонов поиска и доступа.}

  % \workproject{\textbf{Приватный поиск шаблона FPS/RFPS:} Реализовал и проверил
  % прототип приватного поиска шаблона, заменив преобразование сопоставления
  % на основе DFT при приближённой арифметике CKKS преобразованием на основе
  % NTT при точной арифметике BGV/BFV; приватность шаблона в модели HBC-сервера
  % основана на IND-CPA, при этом явно определены утечки протокола.}

  % приватность [запроса/обновления клиента/шаблона] на основе IND-CPA
  % в модели HBC-[узла/агрегатора/сервера]

  \workproject{\textbf{\href{https://iasenovets.github.io/pir-schemes}{Восстановил}}
  модели предварительной обработки, построение запросов, серверные вычисления
  и развёртывание ответов для таких PIR-протоколов, как Piano,
  FrodoPIR/SimplePIR, XPIR, SealPIR и OnionPIR, и проанализировал компромиссы
  между объёмом коммуникации и производительностью.}
}
...
\cvrow{Навыки}{%
  \begin{tabularx}{\linewidth}
    ...
    &
    \skillcell{Криптография:}{%
      на основе решёток, симметричная,
      асимметричная}
    \\[0.20em]
    ...
    \skillcell{Инженерия безопасности:}{%
      YARA, анализ вредоносного ПО (PS/VBA), анализ IoC/TTP,
      MITRE ATT\&CK}
    \\[0.20em]
...
\cvrow{Стажировки\\в индустрии}{%
  \workentry
    {\textbf{Инженер по информационной безопасности} @
    \href{https://global.ptsecurity.com/en/}{Positive Technologies}}
    {авг. 2024--февр. 2025}

  \workproject{Разработал инструменты на Python с покрытием модульными тестами
  ...
  с DAG в Airflow, который собирал образцы из MalwareBazaar, сохранял
  артефакты в S3 и сканировал их с помощью YARA.}
...
\cvrow{Другой опыт}{%
  \workentry
    ...
    {сент. 2023--авг. 2024}
  \workproject{Разработал прикладной уровень образовательной платформы на основе
  LLM, которой пользовались преподаватели и студенты ИТ-направлений вузов:
  приложение на Next.js/TypeScript, сервисы на Python/Flask, слой данных
  PostgreSQL/Prisma и аутентификацию Auth.js.}
  \workproject{Интегрировал рабочие процессы генерации на основе LangChain
  с AsyncIO и Redis для асинхронной оркестрации LLM и управления
  промежуточным состоянием.}
  \workproject{Контейнеризировал приложение с помощью Docker, настроил Nginx,
  развернул его в облачной инфраструктуре VPS и построил процесс CI/CD
  в GitHub Actions.}
}
...
\cvrow{}{%
  ...
  \workproject{Спроектировал и разработал полнофункциональную систему
  QR-инвентаризации склада с использованием Python, FastAPI и MySQL,
  полностью заменив ручной процесс учёта.}

  \workproject{Обеспечил поддержку около 1000 записей и более 20 одновременных
  пользователей; добавил покрытие модульными тестами, разработку с использованием
  Git, отладку и развёртывание в Linux.}
}
...
\cvrow{Награды и\\достижения}{%
  ...
  заняв 11-е место среди более чем 100 команд за разработку решения
  для мониторинга трафика в целях обеспечения безопасности
  низковысотного воздушного пространства.

  \vspace{0.45em}
  \entry
    {\textbf{Диплом первой степени, Severgeoecotech}}
    {2022}
  Занял первое место в секции «Современные информационные технологии»
  на XXIII Международной молодёжной научной конференции.
